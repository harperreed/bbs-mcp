// ABOUTME: Migration command for converting BBS data between storage backends
...
func runMigrate(cmd *cobra.Command, args []string) error {
	...
	// Open target storage
	dst, err := openStorage(targetBackend, targetDataDir)
	if err != nil {
	...
	// Print plan
	color.Yellow("Migrating BBS data:")
...
// openStorage creates a Storage implementation for the given backend and data directory.
func openStorage(backend, dataDir string) (storage.Storage, error) {
	switch backend {
	case "sqlite":
